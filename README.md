...
frontend/run_game_hidden.vbs  桌面快捷方式使用的隐藏启动器
tests/       账号系统回归测试
words/       物理、数学词库 CSV
...
词库 CSV 放在 `words/`，线索 JSON 放在 `clues/`。0.3.0 新增了物理连续介质力学、宇宙学、量子信息、物理噩梦前沿理论，以及数学简单/普通/困难补充词表和数学噩梦词库；0.3.1 清理了新增词表中的机械前缀和非缩写英文混名。新增或维护词条时，建议同步检查：
