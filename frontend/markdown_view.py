...
def render_markdown(parent, content, mode="detail"):
    ...
    muted = "#9ca8c7"
    accent = "#8fb6ff"
    base_size = 15 if mode == "quick" else 10
    ...
    def flush_paragraph():
        ...
        if text:
            ...
            ).pack(anchor="w", fill="x", padx=22, pady=(2, 8))
        paragraph = []
    ...
    while index < len(lines):
        ...
        if stripped.startswith("#"):
            ...
            ).pack(anchor="w", padx=22, pady=(16 if level <= 2 else 10, 6))
            index += 1
            continue
        if stripped.startswith("- "):
            flush_paragraph()
            tk.Label(
                ...
                anchor="w",
                font=("Microsoft YaHei UI", base_size),
            ).pack(anchor="w", fill="x", padx=36, pady=3)
            index += 1
            continue
...
def render_inline_markdown(
    ...
    widget.tag_configure("math", font=("Consolas", max(9, base_size)), foreground="#fff2bd", background="#101827")
...
def _insert_inline_segments(widget, text):
    ...
    while index < len(text):
        ...
        if start > index:
            widget.insert("end", text[index:start])
        widget.insert("end", body, (tag,))
        index = end


def _next_inline_marker(text, start_index):
    candidates = []
    for opener, closer, tag in (("**", "**", "bold"), ("`", "`", "code"), ("$", "$", "math")):
...
def _render_table(parent, table_lines, base_size):
    ...
    table.pack(anchor="w", fill="x", padx=22, pady=(4, 12))
...
def _render_code(parent, code, base_size):
    tk.Label(
        ...
        justify="left",
        anchor="w",
        font=("Consolas", max(9, base_size - 1)),
        padx=14,
        pady=10,
        wraplength=1100,
    ).pack(anchor="w", fill="x", padx=22, pady=(4, 12))
