...
def record_single_rating(record):
    ...
    return round(clamp(rating, 0.0, 20.0), 3)
...
def summarize_rating(records, achievements_data=None, include_achievements=True):
    ...
    play_rating = clamp(0.85 * best_average + 0.15 * recent_average, 0.0, 20.0)
    rating = clamp(play_rating + achievement_part["bonus"], 0.0, 20.0)
    return {
...
def term_cycle_key(term):
    return "|".join(str(part) for part in (term.chinese, term.initials, term.source_label, term.source))


def load_daily_terms_state(today=None):
    current_day = today or datetime.now().date().isoformat()
    if not DAILY_TERMS_FILE.exists():
        return {"date": current_day, "buckets": {}}
    try:
